...
class Trainer:
    def __init__(self, env_type,
        ...
        self.stop_training = False  # 用于控制早停的标志位
    ...
    def get_multi_processor_loader(self, train_ds, val_ds):
        ...
        class BatchGeneratorsAdapter:
            def __init__(self, transform):
                self.transform = transform
            
            def __call__(self, data):
                # MONAI 传入的是 data (dict)
                # 我们把它拆包成 **data 传给 batchgenerators
        ...
        # return data_generator, val_data_generator
            ...
            #                 # 堆叠成 [Batch, Channel, H, W, D]
            #                 return {
            #                     'data': np.stack(data), 
            #                     'seg': np.stack(seg), 
            #                     'properties': [item['properties'] for item in batch],
            #                     'keys': [item.get('key', idx) for idx, item in enumerate(batch)]
            #                 }

            #             # 使用标准的 DataLoader，batch_size=1 (全图通常很大，只能BS=1)，shuffle=False
            #             # 注意：这里不再使用 DataLoaderMultiProcess，因为它会强制 Crop
            # from torch.utils.data import DataLoader as TorchDataLoader
            # val_loader = TorchDataLoader(val_ds, 
            #                             batch_size=1, 
            #                             shuffle=False, 
            #                             num_workers=4, 
            #                             collate_fn=numpy_collate) 
            
            # # LimitedLenWrapper 会负责从 loader 取数据并应用 val_transforms (转 Tensor 等)
            # val_data_generator = LimitedLenWrapper(self.val_number, data_loader=val_loader, transform=val_transforms,
            #                                     num_processes=2, num_cached=3, seeds=None,
            #                                     pin_memory=True, wait_time=0.02)
        # ================== 修改结束 ==================
            val_transforms_adapted = BatchGeneratorsAdapter(val_transforms)
            val_ds_wrapped = Dataset(data=val_ds, transform=val_transforms_adapted)
            val_data_generator = DataLoader(val_ds_wrapped, batch_size=1, shuffle=False, num_workers=4)
        return data_generator, val_data_generator
    ...
    def train(self,
        ...
        if self.scheduler_type == "cosine_with_warmup":
            ...
            # mode='max' 表示指标越高越好（如 Dice），'min' 表示越低越好（如 Loss）
            # patience=10 表示指标 10 次没有改善就降低学习率
            # factor=0.5 表示每次降低为原来的 0.5 倍
            self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                self.optimizer, mode='max', factor=0.5, patience=15, verbose=True, min_lr=1e-6
            )
            print(f"scheduler_type is ReduceLROnPlateau")
