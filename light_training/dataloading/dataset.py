...
def get_loader_from_txt(split_file_path):
    """
    从 txt 文件中读取划分好的数据集路径
    """
    train_datalist = []
    val_datalist = []
    test_datalist = []
    ...
    for line in lines:
        ...
        if "=== Training Set" in line:
            current_list = train_datalist
            continue
        elif "=== Validation Set" in line:
            current_list = val_datalist
            continue
        elif "=== Test Set" in line:
            current_list = test_datalist
            continue
            
        # 添加文件路径
        if current_list is not None:
            # 假设 txt 中的路径是相对路径 (例如 data/fullres/train/xxx.npz)
            # 如果你的脚本运行目录不是项目根目录，可能需要 os.path.join(root_dir, line)
            if os.path.exists(line):
                current_list.append(line)
            else:
                # 尝试修复路径：如果你在 dataset_split_info.txt 里存的是相对路径
                # 但当前运行位置不同，你可能需要手动加前缀，例如：
                # current_list.append(os.path.join("./", line))
                print(f"Warning: File not found: {line}")
                # 依然添加，防止 dataset 报错时难以排查，或者选择跳过
                current_list.append(line)

    print(f"Dataset split loaded from {split_file_path}")
    print(f"Training data: {len(train_datalist)}")
    print(f"Validation data: {len(val_datalist)}")
    print(f"Test data: {len(test_datalist)}")
